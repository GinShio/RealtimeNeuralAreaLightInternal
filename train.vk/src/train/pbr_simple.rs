...
pub fn train(
    ...
    // load json
    let data_json_path = data_dir.join("data_gen_config.json");
    let data_json = std::fs::read_to_string(data_json_path)?;
    let data_config: serde_json::Value = serde_json::from_str(&data_json)?;
    let texture_size = data_config["texture_size"].as_u64().unwrap() as u32;
    let batch_size = data_config["batch_size"].as_u64().unwrap();
    let first_phase_shard_size = data_config["first_phase_shard_size"].as_u64().unwrap();
    let first_phase_shard_count = data_config["first_phase_shard_count"].as_u64().unwrap();
    let second_phase_shard_size = data_config["second_phase_shard_size"].as_u64().unwrap();
    let second_phase_shard_count = data_config["second_phase_shard_count"].as_u64().unwrap();
    ...
    // transform frame input: (8)
    // - latent vector (8)
    // transform frame output: (12)
    // - transform frame (12)
    // decoder input: (43)
    // - latent vector (8)
    // - transform frame (5 (wo, v1, v2, v3, v4) * 3 * 2)
    // - distance (4 (v1, v2, v3, v4))
    // - area (1)
    // decoder output: BRDF (3)
    let decoder_dimensions = [(8, 12), (43, 64), (64, 64), (64, 64), (64, 3)];
    ...
    let first_shard_buffer_size = batch_size * first_phase_shard_size * data_component_size;
    let second_shard_buffer_size =
        texture_total_pixel_size * second_phase_shard_size * data_component_size;

    let first_phase_shard_data_buffer = create_storage_buffer(
        state,
        first_shard_buffer_size * std::mem::size_of::<f32>() as u64,
    )?;
    let first_phase_shard_data_cpu_buffer = create_cpu_storage_buffer(
        state,
        first_shard_buffer_size * std::mem::size_of::<f32>() as u64,
    )?;

    let second_material_buffer_data = fs::read(data_dir.join("second_phase_data.material.bin"))?;
    let second_phase_material_data_buffer =
        create_storage_buffer_with_data(state, second_material_buffer_data)?;

    let second_phase_shard_data_buffer = create_storage_buffer(
        state,
        second_shard_buffer_size * std::mem::size_of::<f32>() as u64,
    )?;
    let second_phase_shard_data_cpu_buffer = create_cpu_storage_buffer(
        state,
        second_shard_buffer_size * std::mem::size_of::<f32>() as u64,
    )?;
    ...
    // Update train descriptor set
    {
        ...
        let data_buffer_info = [vk::DescriptorBufferInfo::default()
            .buffer(first_phase_shard_data_buffer)
            .offset(0)
    ...
    let (first_phase_init_pipeline, first_phase_init_pipeline_layout) = {
        create_compute_pipeline(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/pbr_simple/init_1st.comp.spv"
            )),
    ...
    let (first_phase_train_pipeline, first_phase_train_pipeline_layout) = {
        create_compute_pipeline(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/pbr_simple/train_1st.comp.spv"
            )),
    ...
    let (first_phase_optimize_pipeline, first_phase_optimize_pipeline_layout) = {
        create_compute_pipeline(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/pbr_simple/adam_1st.comp.spv"
            )),
    ...
    // Update train descriptor set
    {
        ...
        let material_data_buffer_info = [vk::DescriptorBufferInfo::default()
            ...
            .offset(0)
            .range(second_material_buffer_size * std::mem::size_of::<f32>() as u64)];
        let train_data_buffer_info = [vk::DescriptorBufferInfo::default()
            .buffer(second_phase_shard_data_buffer)
            .offset(0)
            .range(second_shard_butter_size * std::mem::size_of::<f32>() as u64)];
        ...
        let descriptor_writes = [
            ...
            vk::WriteDescriptorSet::default()
                ...
                .dst_binding(6)
                .descriptor_type(vk::DescriptorType::STORAGE_BUFFER)
                .buffer_info(&train_data_buffer_info),
        ];
        unsafe { state.device.update_descriptor_sets(&descriptor_writes, &[]) };
    }
    ...
    let (second_phase_init_pipeline, second_phase_init_pipeline_layout) = {
        create_compute_pipeline(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/pbr_simple/init_2nd.comp.spv"
            )),
    ...
    let (second_phase_train_pipeline, second_phase_train_pipeline_layout) = {
        create_compute_pipeline(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/pbr_simple/train_2nd.comp.spv"
            )),
    ...
    let (second_phase_optimize_pipeline, second_phase_optimize_pipeline_layout) = {
        create_compute_pipeline(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/pbr_simple/adam_2nd.comp.spv"
            )),
    ...
    for i in 0..epochs {
        ...
        // load shard data
        let mollification_shard_count = first_phase_shard_count / 15;
        let data = if i < mollification_shard_count {
            fs::read(data_dir.join(format!("first_phase_data-mollified.shard-{}.bin", 1))).unwrap()
        } else {
            let index = (i - mollification_shard_count)
                % (first_phase_shard_count - mollification_shard_count);
            fs::read(data_dir.join(format!("first_phase_data.shard-{}.bin", index))).unwrap()
        };
        first_phase_data_cpu_buffer_allocation
            .mapped_slice_mut()
        ...
        for j in 0..first_phase_shard_count {
            let seed = rng.random();
    ...
    for i in 0..epochs {
        ...
        // load shard data
        let data = {
            let index = i % second_phase_shard_count;
            fs::read(data_dir.join(format!("second_phase_data.shard-{}.bin", index))).unwrap()
        };
        second_phase_data_cpu_buffer_allocation
            .mapped_slice_mut()
            .expect("Failed to map first phase data buffer")
            .copy_from_slice(&data);
        let command_buffer = state.begin_single_time_commands();
        ...
        for j in 0..second_phase_shard_count {
            let seed = rng.random();
