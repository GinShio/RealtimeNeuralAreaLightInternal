...
pub fn save_mip_image(data: &[u8], width: u32, save_dir: impl AsRef<Path>) -> Result<()> {
    ...
    while current_mip_width > 0 {
        ...
        let mip_size = pixel_count * 4 * std::mem::size_of::<f16>();
        ...
        let f16_pixels: Vec<[f16; 4]> = mip_data
            .chunks_exact(8) // 4ch * 2bytes
            .map(|chunk| {
                [
                    f16::from_bits(u16::from_le_bytes([chunk[0], chunk[1]])),
                    f16::from_bits(u16::from_le_bytes([chunk[2], chunk[3]])),
                    f16::from_bits(u16::from_le_bytes([chunk[4], chunk[5]])),
                    f16::from_bits(u16::from_le_bytes([chunk[6], chunk[7]])),
                ]
            })
            .collect();

        let mip_path = save_dir
            .as_ref()
            .join(format!("latent-texture.{}.exr", current_mip_width));
