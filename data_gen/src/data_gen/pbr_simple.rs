...
pub fn data_gen(
    ...
    let first_shard_buffer_size = batch_size * first_phase_shard_size * parameter_size;
    let second_shard_buffer_size =
        texture_total_pixel_size * second_phase_shard_size * parameter_size;
    ...
    for i in 0..first_phase_shard_count {
        ...
        let mollification_scale = 1.0 - (i as f32 / mollification_shard_count as f32).max(1.0);
        ...
        // save shard data
        {
            ...
            let mut file = File::create(format!(
                "train_data/pbr-simple/first_phase_data{}.shard-{}.bin",
                if mollification_scale > 0.0 {
                    "-mollified"
                } else {
                    ""
                },
    ...
    let uniform_data = SecondPhaseUniformBuffer {
        data_size: second_phase_shard_size as u32,
        texture_size: texture_size,
        max_light_size,
        max_light_distance,
    };
    ...
    for i in 0..second_phase_shard_count {
        ...
        // save shard data
        {
            ...
            let mut file = File::create(format!(
                "train_data/pbr-simple/second_phase_data.shard-{}.bin",
                i
            ))?;
            file.write_all(data_slice)
                .expect("Failed to write first phase data shard");
