...
pub fn load_glb(state: &mut VulkanState, path: impl AsRef<Path>) -> Result<Vec<ModelData>> {
    ...
    for primitive in mesh.primitives() {
        ...
        let tangents = if let Some(tangents) = reader.read_tangents() {
            tangents.map(glam::Vec4::from).collect::<Vec<_>>()
        } else {
            ...
            for is in indices.chunks(3) {
                ...
                let bitangnet = ((edge2 * delta_uv1.x - edge1 * delta_uv2.x) * r).normalize();
        ...
        for i in 0..positions.len() {
            ...
            //     positions[i], normals[i], tangents[i], uvs[i]
            // );
        ...
        model_data.push(ModelData::new(state, &vertices, &indices)?);
    }
