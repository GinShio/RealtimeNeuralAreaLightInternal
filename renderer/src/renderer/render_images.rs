...
pub struct RenderImages {
    ...
    pub depth_scene_images: Vec<vk::Image>,
    pub depth_scene_image_views: Vec<vk::ImageView>,
    depth_scene_image_allocations: Vec<Allocation>,
...
impl RenderImages {
    ...
    pub fn new(state: &mut VulkanState) -> Result<Self> {
        ...
        Ok(Self {
            ...
            depth_scene_images,
            depth_scene_image_views,
            depth_scene_image_allocations,
            after_tone_mapping_images,
            after_tone_mapping_image_views,
            after_tone_mapping_image_allocations,
        })
    ...
    pub fn destroy(&mut self, state: &mut VulkanState) {
        unsafe {
            ...
            for image_view in &self.depth_scene_image_views {
                state.device.destroy_image_view(*image_view, None);
            }
            for image in &self.depth_scene_images {
                state.device.destroy_image(*image, None);
            }
            for allocation in self.depth_scene_image_allocations.drain(..) {
                state
                    .allocator()
                    .free(allocation)
                    .expect("Failed to free depth scene image allocation");
            }
